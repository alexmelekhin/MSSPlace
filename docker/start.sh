...
if [ $ARCH == "x86_64" ]; then
    if command -v nvidia-smi &> /dev/null; then
        ...
        echo "${orange}CPU-only${reset_color} build is not supported"
        exit 1
    fi
else
...
docker run -it -d --rm \
    ...
    --name ${USER}_mssplace \
    --net host \
    -v $PROJECT_ROOT_DIR:/home/docker_mssplace/MSSPlace:rw \
    -v $DATASETS_DIR:/home/docker_mssplace/Datasets:rw \
    mssplace:latest
