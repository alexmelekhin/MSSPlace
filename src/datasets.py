...
def collate_data_dict(
    dataset: BasePlaceRecognitionDataset, data_list: List[Dict[str, Tensor]]
) -> Dict[str, Tensor]:
    ...
    for data_key in data_list[0].keys():
        if data_key == "idx":
        ...
        elif data_key.startswith("text_"):
            result[f"texts_{data_key[5:]}"] = torch.stack([e[data_key] for e in data_list])
            ...
            if dataset.pointcloud_set_transform is not None:
                # Apply the same transformation on all dataset elements
                coords_tensor = dataset.pointcloud_set_transform(coords_tensor)
            ...
            for coords, feats in zip(coords_list, feats_list):
                quantized_coords, quantized_feats = ME.utils.sparse_quantize(
                    coordinates=coords,
                    features=feats,
                    quantization_size=dataset._pointcloud_quantization_size,
                )
...
class NCLTDatasetWithText(BasePlaceRecognitionDataset):
    """NCLT dataset implementation with text embeddings."""
    ...
    def __init__(
        ...
        text_embeddings_dirname: str = "clip-vit-base-patch32",
        ...
        Args:
            ...
            text_embeddings_dirname (str): Text embeddings directory name. Defaults to "clip-vit-base-patch32".
        ...
        if any(elem.startswith("text") for elem in self.data_to_load):
            self._text_embeddings_dirname = text_embeddings_dirname
            if not (self.dataset_root / _track_name / self._text_embeddings_dirname).exists():
                raise FileNotFoundError(
                    f"Text embeddings directory {self._text_embeddings_dirname!r} does not exist."
                )
    ...
    def __getitem__(self, idx: int) -> Dict[str, Tensor]:  # noqa: D105
        ...
        for data_source in self.data_to_load:
            ...
            elif data_source.startswith("text_"):
    ...
    def _load_pc(self, filepath: Union[str, Path]) -> Tensor:
        ...
        pc = np.fromfile(filepath, dtype=np.float32).reshape(-1, 3)
    ...
    def collate_fn(self, data_list: List[Dict[str, Tensor]]) -> Dict[str, Tensor]:
        ...
        """
        return collate_data_dict(self, data_list)


class OxfordDatasetWithText(BasePlaceRecognitionDataset):
    """PointNetVLAD Oxford RobotCar dataset implementation with text embeddings."""
